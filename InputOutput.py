...
# PREPRO OBS 
# Header
PreproHdr = "# SOD DOY C PRN    ELEV     AZIM  VALID REJ  STATUS    C1          C1SMOOTHED          L1           S1     CODERATE   CODEACC    PHASERATE PHASEACC  GEOMFREE VTECRATE  iAATR\n"

# Line format
PreproFmt = "%05d %03d %s %02d %8.3f %8.3f %4d %4d %4d %15.3f %15.3f %15.3f %8.3f %10.3f %10.3f %10.3f %10.3f %8.3f %8.3f %8.3f".split()
...
# CORR 
# Header
CorrHdr = "#SOD DOY C PRN   ELEV    AZIM     IPPLON   IPPLAT   FLAG  SAT-X         SAT-Y           SAT-Z            SAT-CLK      UISD     STD      CORR-PSR       GEOM-RNGE     PSR-RES       RCVR-CLK     SFLT     SUIRE    STROPO   SAIR  SNOISEDIV  SMP    SUERE   ENTGPS \n"

# Line format
CorrFmt = "%05d %03d %1s %02d %8.3f %8.3f %8.3f %8.3f %4d %14.3f %14.3f %14.3f %14.3f %8.3f %8.3f %14.3f %14.3f %10.4f %14.3f %10.4f %8.4f %8.4f %8.4f %7.3f %7.3f %7.3f %8.4f %8.4f".split()
        ...
        if(isinstance(LowLim[i], int) or \
            ...
            try:
                if(Field<LowLim[i] or Field>UppLim[i]):
                    # Out of range
                    sys.stderr.write("ERROR: Configuration parameter %s %f is out of range [%f, %f]\n" % (Key, Field, LowLim[i], UppLim[i]))
...
def readConf(CfgFile):
        ...
        for Line in Lines:
            ...
            if Line[0]!='#':
                ...
                if Fields != None :
                    ...
                    if len(Fields) == 1:
                        sys.stderr.write("ERROR: Configuration file contains a parameter with no value: " + Line) 
                        sys.exit(-1) 
                    ...
                    # if the line contains a conf parameter
                        ...
                        # VPE Histogram outputs selection [0:OFF|1:ON]
                        #--------------------------------------------------------------------       
                        elif Key=='VPEHIST_OUT':
                            # Check parameter and load it in Conf
                            Conf[Key] = checkConfParam(Key, Fields, 1, 1, [0], [1])
    ...
    # # Check number of conf parameters (when the number is fixed)
    # if (NReadParams != 39):
    #     # Raise error
...
def processConf(Conf):
    ...
    for Key in ConfCopy:
        ...
        if Key == "INI_DATE" or Key == "END_DATE":
            ...
            # Compute Julian Day
            Conf[Key + "_JD"] = int(round(convertYearMonthDay2JulianDay(int(ParamSplit[2]), int(ParamSplit[1]), int(ParamSplit[0]))))
                    ...
                    # if the line contains a conf parameter
                        ...
                        if isinstance(Acr, str) and (len(Acr) <= 4):
                            ...
                            if Rcvr[RcvrIdx["FLAG"]] == 1.0:
                                ...
                                Rcvr.append(llh2xyz(float(Rcvr[RcvrIdx["LON"]]), float(Rcvr[RcvrIdx["LAT"]]), float(Rcvr[RcvrIdx["ALT"]])))
...
def openInputFile(Path):
    ...
    # Display Message
    print("INFO: Reading file: %s..." % Path)
...
def generatePosFile(fpos, PosInfo, Rcvr):
    ...
    # Purpose: generate output file with Position results
    ...
    # PosInfo: dict
    #          Dictionary containing Pos info for Rcvr in the 
    #          current epoch
